...
def get_cnx(database: str = "") -> mysql.connector.MySQLConnection:
...
def migrate():
    ...
    cnx = get_cnx()
    cursor = cnx.cursor()
    cursor.execute("CREATE DATABASE IF NOT EXISTS db_internet_monitoring")
    cnx.commit()
    cnx.database = 'db_internet_monitoring'
...
def main():
    ...
    try:
        cnx = get_cnx("db_internet_monitoring")
